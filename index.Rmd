...
author: "[Lety Kemp](https://twitter.com/letykemp) | [Masters of Health Data Science](http://www.swansea.ac.uk/medicine/learningandteaching/msc-health-data-science/)"
date: " "
output:
  html_document:
    theme: cosmo
...
This page shows the methodology of an exploratory data analysis where I created risk rates of accident severity type 1 in the region of Swansea, UK. 

Adjusted risk rates were calculated and illustrated in a quantile map. The end product was a academic conference poster presented as part of the Data Visualisation module. More details on [poster](Poster_A3_DataViz.pdf). (This site was not part of the assignment, however, I loved working with RStudio and just wanted more!! ♥♥♥)

The data sources were Open Data from UK authorities, but directly retrieved from [this](http://richfry.github.io/Exposure.html#introduction) tutorial by GIS specialist [Richard Fry](https://twitter.com/richfry). Richard was responsible for the GIS component of the Data Visualisation module.

The analysis and map were created using R software. This site was also created using RStudio, Rmarkdown and GitHub. The [poster](Poster_A3_DataViz.pdf) was created in Adobe Illustrator.


Following is the R code I used to create the rates and plot them in the map:
```{r setup_code, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
...
welsh.lsoa <- readOGR(dsn = "wales_lsoa_2011_gen.shp", layer = "wales_lsoa_2011_gen", p4s = bng)

```

Load road summary to obtain the total road Kilometers in order to calculate the risk rates


```{r roads_code, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
# load roads file
...
Load Stats19, and census files prepared by Richard Fry in [this tutorial](http://richfry.github.io/Exposure.html#introduction) 

```{r swansea, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
...
```
Join injury rates and visualise in Swansea map 

```{r injury, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
...
```{r NA, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
...
Calculate adjusted rates for severity type 1 accidents by km of road and show on final map

```{r final, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
...
The above map was used on final [poster](Poster_A3_DataViz.pdf)
